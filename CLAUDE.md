...
### 국내(한국) 기준이 기본값 — 상황별 스킬 분담
국내 현장·한국어 요청·국내 용어('산안법·안전보건규칙·중대재해·중대재해처벌법·KOSHA·산재'
등)면 아래 **국내 스킬**을 쓴다. 용도별로 나눈다.

1. **점검표 생성·산재/중대재해 자동 판정·TBM(스크립트 기반)** → **`safety-inspection-kr`**
   (공종별 체크리스트 생성기, 사고 입력→보고 대상 판정 스크립트 포함)
2. **컴플라이언스·부적합 판정(심각도 4단계)·시정조치·안전 보고/컴플라이언스 문서** →
   **`construction-safety`** (KOSHA GUIDE 연계, Finding→심각도→시정조치 구조)
3. 두 스킬 모두 국내 법령(산안법·안전보건규칙·중대재해처벌법) 기반이므로, 요청 성격에
   맞는 쪽을 고르고 필요하면 **함께** 사용한다.

### 해외/미국(OSHA) 기준
- 미국(OSHA) 기준을 명시하거나 해외 현장/영문 규정(OSHA, 29 CFR 1926 등)을 요구하면
  → 아래 DDC 스킬(미국 OSHA 기반)을 사용한다.

### 공통 규칙
- 이 안전 규칙은 다른 스킬 규칙(HWPX·프레젠테이션·모바일)보다 **우선**한다. 단, 안전 문서를
...
| 스킬 이름 | 기준 | 용도 | 디렉터리 |
| --- | --- | --- | --- |
| `safety-inspection-kr` | **국내(한국)** | 공종별 점검표 생성·산재/중대재해 보고 자동판정·TBM (스크립트 기반) | `.claude/skills/safety-inspection-kr` |
| `construction-safety` | **국내(한국)** | 컴플라이언스·부적합 판정(심각도 4단계)·시정조치·안전 보고/문서 (KOSHA GUIDE 연계) | `.claude/skills/construction-safety` |
